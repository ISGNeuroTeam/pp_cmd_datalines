...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"datalines_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f datalines-*.tar.gz
	echo Create archive \"datalines-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../datalines-$(VERSION)-$(BRANCH).tar.gz datalines

clean_pack:
	rm -f datalines-*.tar.gz


datalines.tar.gz: build
	cd make_build; tar czf ../datalines.tar.gz datalines && rm -rf ../make_build
...
dev: pp_cmd otl_v1_config.ini
	ln -s -r ./datalines pp_cmd/datalines
